var CPU = function () {
  ...
  var microcode = {
    ...
    0x05: function (offset) {
      // BEQ
      if (accumulator == 0) {
        pc = (pc + offset - 1) & byte_mask;
      }
    },
    0x06: function (address) {
      // ANDm
      accumulator &= me.read_byte(address);
    },
    0x07: function (address) {
      // LOADm
      accumulator = me.read_byte(address);
    },
    0x09: function (immediate) {
      // SUBi
      accumulator = (accumulator - immediate) & byte_mask;
    },
    0x0a: function (immediate) {
      // ADDi
      accumulator = (accumulator + immediate) & byte_mask;
    },
    0x0b: function (immediate) {
      // ORi
    ...
    0x0e: function (immediate) {
      // ANDi
      accumulator &= immediate;
    },
    0x0f: function (immediate) {
      // LOADi
      accumulator = immediate;
    },
  };
  ...
  this.step = function () {
    if (!this.is_valid_address(pc + 1)) {
      error("Invalid PC: " + Listing.hex(pc));
      return;
    }
    ...
    if (!microcode[opcode]) {
      error("Invalid opcode: " + Listing.hex(opcode));
      return;
    }
  ...
  this.read_byte = function (address) {
    if (!this.is_valid_address(address)) {
      error("Attempted read at invalid address: " + Listing.hex(address));
      return;
    }
  ...
  this.write_byte = function (address, data) {
    if (!this.is_valid_address(address)) {
      error("Attempted write at invalid address: " + Listing.hex(address));
      return;
    }
    memory[address] = data;
  };
